 ...
 *
 * IMPORTANT: This file lives inside functions/_lib/ so that Base44 Functions
 * (Deno runtime) can import it. Do NOT import from /src/components/ or .jsx.
 *
...
export async function simulateDayCore(base44: any, options: any = {}) {
  const { tierFilter = null, isCron = false } = options;
  const tiersToProcess = tierFilter ? [tierFilter] : EFL_TIERS;
  const summary: any = { tiers: {}, isCron, timestamp: new Date().toISOString() };
  ...
  for (const tier of tiersToProcess) {
    try {
      ...
      // ── 4. Classify fixtures ───────────────────────
      // Robust: a fixture is unplayed if it has no played_at AND no goals recorded.
      const upcomingFixtures = allFixtures.filter(
        (f: any) => !f.played_at && f.home_goals == null && f.away_goals == null
      );

      const playedFixtures = allFixtures.filter(
        (f: any) => f.played_at != null || f.status === 'PLAYED'
      );
      ...
      if (confirmedPlayed.length !== EXPECTED_FIXTURES_PER_MATCHDAY) {
        ...
        const unconfirmed = verifyFixtures.filter((f: any) => !f.played_at && f.status !== 'PLAYED');
      ...
      const standingUpdates: Record<string, any> = {};
      ...
      // ── 9. Advance matchday ────────────────────────
      // LAST step. Only reached if results + standings are confirmed.
      await base44.asServiceRole.entities.SeasonProgress.update(progress.id, {
        current_matchday: matchday + 1,
      });
      await base44.asServiceRole.entities.Season.update(season.id, {
  ...
  // ── Final summary ──────────────────────────────────
  const lines = Object.entries(summary.tiers).map(([t, r]: [string, any]) => {
